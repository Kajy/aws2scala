...
object SnsScalaCheckImplicits {
  ...
  implicit lazy val shrinkAddPermissionRequest: Shrink[AddPermissionRequest] =
    Shrink { request =>
      ...
      Shrink.shrink(arn).map(x => request.copy(topicArn = x.arnString)) lazyAppendedAll
        Shrink.shrink(request.label).filter(_.nonEmpty).map(x => request.copy(label = x)) lazyAppendedAll
        Shrink.shrink(accounts).filter(_.nonEmpty).map(x => request.copy(accounts = x.map(_.id))) lazyAppendedAll
  ...
  implicit lazy val shrinkPlatformApplication: Shrink[PlatformApplication] =
    Shrink { application =>
      ...
      Shrink.shrink(applicationArn).map(x => application.copy(arn = x.arnString)) lazyAppendedAll
        shrinkArn("EventEndpointCreated", application.eventEndpointCreated.map(TopicArn.fromArnString(_))) lazyAppendedAll
        shrinkArn("EventEndpointDeleted", application.eventEndpointDeleted.map(TopicArn.fromArnString(_))) lazyAppendedAll
        shrinkArn("EventEndpointUpdated", application.eventEndpointUpdated.map(TopicArn.fromArnString(_))) lazyAppendedAll
        shrinkArn("EventDeliveryFailure", application.eventDeliveryFailure.map(TopicArn.fromArnString(_))) lazyAppendedAll
        shrinkArn("SuccessFeedbackRoleArn", application.successFeedbackRoleArn.map(RoleArn.fromArnString(_))) lazyAppendedAll
        shrinkArn("FailureFeedbackRoleArn", application.failureFeedbackRoleArn.map(RoleArn.fromArnString(_))) lazyAppendedAll
    ...
    Shrink { request =>
      ...
      val shrunkStructure =
        if (request.messageStructure.isDefined) {
          request.copy(messageStructure = None) #:: LazyList.empty
        } else {
          LazyList.empty
        }

      shrunkArn lazyAppendedAll shrunkMessage lazyAppendedAll shrunkAttributes lazyAppendedAll shrunkStructure
    }
