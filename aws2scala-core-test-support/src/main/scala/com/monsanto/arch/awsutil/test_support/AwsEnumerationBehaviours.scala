...
trait AwsEnumerationBehaviours { this: AnyFreeSpec =>
  def anAwsEnumeration[AwsType,ScalaType](awsValueArray: Array[AwsType],
    ...
    val awsValues = Table("AWS value", awsValueArray.toIndexedSeq: _*)
    val scalaValues = Table("Scala value", scalaValueSeq.toIndexedSeq: _*)
  ...
  def anAwsEnumeration[ScalaType <: AwsEnumeration[AwsType], AwsType <: Enum[AwsType]: ClassTag](companion: AwsEnumerationCompanion[ScalaType,AwsType]): Unit = {
    val awsValues = {
      ...
      Table("AWS value", values.toIndexedSeq: _*)
    }
