...
private[awsutil] class DefaultAsyncSecurityTokenServiceClient(streaming: StreamingSecurityTokenServiceClient) extends AsyncSecurityTokenServiceClient {
  override def assumeRole(roleArn: String, sessionName: String)(implicit as: ActorSystem) =
    Source.single(AssumeRoleRequest(roleArn, sessionName))
      .via(streaming.roleAssumer)
      .map(_.credentials)
      .runWith(Sink.head)

  override def assumeRole(roleArn: String, sessionName: String, externalId: String)(implicit as: ActorSystem) =
    Source.single(AssumeRoleRequest(roleArn, sessionName, externalId = Some(externalId)))
      .via(streaming.roleAssumer)
      .map(_.credentials)
      .runWith(Sink.head)

  override def assumeRole(request: AssumeRoleRequest)(implicit as: ActorSystem) =
