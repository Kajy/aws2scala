...
trait AsyncS3Client extends AsyncAwsClient {
  /** Creates a new bucket with the given name.  The location of the bucket is dependent on the location for which the
    ...
    */
  def createBucket(bucketName: String)(implicit as: ActorSystem): Future[Bucket]
  ...
  /** Deletes the bucket with the given name. This bucket must be empty
    ...
    */
  def deleteBucket(bucketName: String)(implicit as: ActorSystem): Future[String]

  /** Empties the bucket with the given name.
    *
    * @return a future containing the name of the bucket that was emptied out
    */
  def emptyBucket(bucketName: String)(implicit as: ActorSystem):Future[String]
  ...
  /** Deletes the bucket with the given name, and all its contents
    ...
    * @return a future containing the name of the bucket that was deleted
    */
  def emptyAndDeleteBucket(bucketName:String)(implicit as: ActorSystem) :Future[String]

  /** Returns a sequence of all of the buckets available. */
  def listBuckets()(implicit as: ActorSystem): Future[Seq[Bucket]]

  /** Returns whether the bucket exists. */
  def doesBucketExist(bucketName: String)(implicit as: ActorSystem): Future[Boolean]
  ...
  /** Returns the given bucket‘s policy, if any.
    ...
    */
  def getBucketPolicy(bucketName: String)(implicit as: ActorSystem): Future[Option[String]]
  ...
  /** Sets the policy for the given bucket.
    ...
    */
  def setBucketPolicy(bucketName: String, policyText: Option[String])(implicit as: ActorSystem): Future[String]
  ...
  /** Sets the policy for the given bucket.
    ...
    * @return a future containing the name of the bucket whose policy was modified
    */
  def setBucketPolicy(bucketName: String, policyText: String)(implicit as: ActorSystem): Future[String]
  ...
  /** Deletes the policy for the given bucket.
    ...
    */
  def deleteBucketPolicy(bucketName: String)(implicit as: ActorSystem): Future[String]

  /** Gets the tagging configuration for the given bucket. */
  def getBucketTags(bucketName: String)(implicit as: ActorSystem): Future[Map[String,String]]
  ...
  /** Sets the tagging configuration for the given bucket.  If the tags map is empty, this function is equivalent to
    ...
    */
  def setBucketTags(bucketName: String, tags: Map[String,String])(implicit as: ActorSystem): Future[String]

  /** Removes the tagging configuration for the given bucket.
    *
    * @return a future containing the bucket name whose tagging was set
    */
  def deleteBucketTags(bucketName: String)(implicit as: ActorSystem): Future[String]
  ...
  /** Uploads the content to the given bucket and key.  Once the transfer is complete, it will wait until S3 lists the
    ...
    * file.
    */
  def upload[T: UploadSource](bucketName: String, key: String, t: T)(implicit as: ActorSystem): Future[aws.S3ObjectSummary]

  /** Downloads the object at the given bucket and key to an object.  By default, the sink type may be a string or a
    * byte array.
    */
  def download[T: DownloadSink](bucketName: String, key: String)(implicit as: ActorSystem): Future[T]

  /** Downloads the object at the given bucket and key to a specific file. */
  def downloadTo(bucketName: String, key: String, file: File)(implicit as: ActorSystem): Future[File]
  ...
  def copy(bucketName: String, sourceKey: String, destKey: String)
          (implicit as: ActorSystem): Future[aws.S3ObjectSummary]
  ...
  def listObjects(bucketName: String)(implicit as: ActorSystem): Future[Seq[aws.S3ObjectSummary]]

  /** List all the objects in a bucket matching the given prefix*/
  def listObjects(bucketName: String, prefix: String)(implicit as: ActorSystem): Future[Seq[aws.S3ObjectSummary]]

  /** Deletes an object. */
  def deleteObject(bucketName: String, key: String)(implicit as: ActorSystem): Future[BucketNameAndKey]

  /** Returns the URL for the object in the given bucket at the given key. */
  def getUrl(bucketName: String, key: String)(implicit as: ActorSystem): Future[URL]
}
