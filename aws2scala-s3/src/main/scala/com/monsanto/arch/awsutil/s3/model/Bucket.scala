...
case class Bucket(name: String, owner: Owner, creationDate: Date) {
  ...
  /** Empties the bucket. */
  def empty()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] = {
    import as.dispatcher
    asyncS3Client.emptyBucket(name).map(_ => ())
  }

  /** Empties the bucket and then attempts to delete it. */
  def emptyAndDelete()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] = {
    import as.dispatcher
    asyncS3Client.emptyAndDeleteBucket(name).map(_ => ())
  }

  /** Checks to see if the bucket, in fact, exists. */
  def exists()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Boolean] =
    asyncS3Client.doesBucketExist(name)

  /** Gets the policy for the bucket, if any. */
  def getPolicy()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Option[String]] =
  ...
  /** Sets or deletes the policy for the bucket.
    ...
    *                   be deleted
    */
  def setPolicy(policyText: Option[String])(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] = {
    import as.dispatcher
  ...
  /** Sets the policy for the bucket.
    ...
    */
  def setPolicy(policyText: String)(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] =
    setPolicy(Option(policyText))

  /** Deletes the bucket‘s policy. */
  def deletePolicy()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] = setPolicy(None)

  /* Gets the tags for the bucket.  If the bucket has no tags, it will return an empty map. */
  def getTags()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Map[String,String]] =
  ...
  def setTags(tags: Map[String,String])(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] =
    asyncS3Client.setBucketTags(name, tags).map(_ => ())(as.dispatcher)

  /** Deletes the tags from the bucket. */
  def deleteTags()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Unit] =
    asyncS3Client.deleteBucketTags(name).map(_ => ())(as.dispatcher)

  /** Returns all the objects in the bucket. */
  def list()(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Seq[Object]] = {
    import as.dispatcher
    asyncS3Client.listObjects(name).map(_.map(Implicits.fromAws))
  }

  /** Returns all the objects in the bucket with the given prefix. */
  def list(prefix: String)(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Seq[Object]] = {
    import as.dispatcher
    asyncS3Client.listObjects(name, prefix).map(_.map(Implicits.fromAws))
  }

  /** Uploads the content to the given key in this bucket. */
  def upload[T: UploadSource](key: String, content: T)(implicit asyncS3Client: AsyncS3Client, as: ActorSystem): Future[Object] =
    asyncS3Client.upload(name, key, content).map(Implicits.fromAws)(as.dispatcher)
}
