...
private[awsutil] class DefaultAsyncEC2Client(client: StreamingEC2Client) extends AsyncEC2Client {
  override def createKeyPair(name: String)(implicit as: ActorSystem) =
    Source.single(name)
      .via(client.keyPairCreator)
      .runWith(Sink.head)

  override def deleteKeyPair(name: String)(implicit as: ActorSystem) =
    Source.single(name)
      .via(client.keyPairDeleter)
      .runWith(Sink.head)

  override def describeKeyPairs()(implicit as: ActorSystem) =
    Source.single(DescribeKeyPairsRequest.allKeyPairs)
      .via(client.keyPairsDescriber)
      .runWith(Sink.seq)

  override def describeKeyPairs(filters: Map[String, Seq[String]])(implicit as: ActorSystem) =
    Source.single(DescribeKeyPairsRequest.filter(filters))
      .via(client.keyPairsDescriber)
      .runWith(Sink.seq)

  override def describeKeyPair(name: String)(implicit as: ActorSystem) =
  ...
  override def describeInstances()(implicit as: ActorSystem) =
    Source.single(DescribeInstancesRequest.allInstances)
      .via(client.instancesDescriber)
      .mapConcat(_.instances.toList)
      .runWith(Sink.seq)

  override def describeInstances(filters: Map[String, Seq[String]])(implicit as: ActorSystem) =
    Source.single(DescribeInstancesRequest.filter(filters))
      .via(client.instancesDescriber)
      .mapConcat(_.instances.toList)
      .runWith(Sink.seq)

  override def describeInstance(instanceId: String)(implicit as: ActorSystem) =
