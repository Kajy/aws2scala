...
object StsScalaCheckImplicits {
  ...
  implicit lazy val shrinkAssumeRoleRequest: Shrink[AssumeRoleRequest] =
    Shrink { request =>
      ...
      Shrink.shrink(roleArn).map(x => request.copy(roleArn = x.arnString)) lazyAppendedAll
        Shrink.shrink(request.roleSessionName).filter(_.length > 1).map(x => request.copy(roleSessionName = x)) lazyAppendedAll
        Shrink.shrink(request.duration).map(x => request.copy(duration = x)) lazyAppendedAll
        Shrink.shrink(request.externalId).filter(_.forall(_.length > 1)).map(x => request.copy(externalId = x)) lazyAppendedAll
        Shrink.shrink(policy).map(p => request.copy(policy = p.map(_.toJson))) lazyAppendedAll
  ...
  implicit lazy val shrinkAssumeRoleResult: Shrink[AssumeRoleResult] =
    Shrink { result =>
      Shrink.shrink(result.assumedRoleUser).map(x => result.copy(assumedRoleUser = x)) lazyAppendedAll
