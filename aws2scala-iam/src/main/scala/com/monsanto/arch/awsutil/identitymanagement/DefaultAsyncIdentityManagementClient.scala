...
private[awsutil] class DefaultAsyncIdentityManagementClient(streaming: StreamingIdentityManagementClient) extends AsyncIdentityManagementClient {
  override def createRole(roleName: String, assumeRolePolicy: Policy)(implicit as: ActorSystem) =
    Source.single(CreateRoleRequest(roleName, assumeRolePolicy, None))
      .via(streaming.roleCreator)
      .runWith(Sink.head)

  override def createRole(roleName: String, assumeRolePolicy: Policy, path: Path)(implicit as: ActorSystem) =
    Source.single(CreateRoleRequest(roleName, assumeRolePolicy, Some(path)))
      .via(streaming.roleCreator)
      .runWith(Sink.head)

  override def deleteRole(roleName: String)(implicit as: ActorSystem) =
    Source.single(roleName)
      .via(streaming.roleDeleter)
      .runWith(Sink.ignore)

  override def listRoles()(implicit as: ActorSystem) =
    Source.single(ListRolesRequest.allRoles)
      .via(streaming.roleLister)
      .runWith(Sink.seq)

  override def listRoles(prefix: Path)(implicit as: ActorSystem) =
    Source.single(ListRolesRequest.withPrefix(prefix))
      .via(streaming.roleLister)
      .runWith(Sink.seq)

  override def attachRolePolicy(roleName: String, policyArn: PolicyArn)(implicit as: ActorSystem) =
    Source.single(AttachRolePolicyRequest(roleName, policyArn))
      .via(streaming.rolePolicyAttacher)
      .runWith(Sink.ignore)

  override def detachRolePolicy(roleName: String, policyArn: PolicyArn)(implicit as: ActorSystem) =
    Source.single(DetachRolePolicyRequest(roleName, policyArn))
      .via(streaming.rolePolicyDetacher)
      .runWith(Sink.ignore)

  override def listAttachedRolePolicies(roleName: String)(implicit as: ActorSystem) =
    Source.single(ListAttachedRolePoliciesRequest(roleName))
      .via(streaming.attachedRolePolicyLister)
      .runWith(Sink.seq)

  override def listAttachedRolePolicies(roleName: String, prefix: Path)(implicit as: ActorSystem) =
    Source.single(ListAttachedRolePoliciesRequest(roleName, prefix))
      .via(streaming.attachedRolePolicyLister)
      .runWith(Sink.seq)

  override def getCurrentUser()(implicit as: ActorSystem) =
    Source.single(GetUserRequest.currentUser)
      .via(streaming.userGetter)
      .runWith(Sink.head)

  override def getUser(name: String)(implicit as: ActorSystem) =
    Source.single(GetUserRequest.forUserName(name))
      .via(streaming.userGetter)
      .runWith(Sink.head)

  override def createPolicy(name: String, document: Policy)(implicit as: ActorSystem) =
    Source.single(CreatePolicyRequest(name, document, None, Path.empty))
      .via(streaming.policyCreator)
      .runWith(Sink.head)

  override def createPolicy(name: String, document: Policy, description: String)(implicit as: ActorSystem) =
  ...
  override def deletePolicy(policyArn: PolicyArn)(implicit as: ActorSystem) =
    Source.single(policyArn)
      .via(streaming.policyDeleter)
      .runWith(Sink.ignore)

  override def getPolicy(policyArn: PolicyArn)(implicit as: ActorSystem) =
    Source.single(policyArn)
      .via(streaming.policyGetter)
      .runWith(Sink.head)

  override def listPolicies()(implicit as: ActorSystem) =
    listPolicies(ListPoliciesRequest.allPolicies)

  override def listPolicies(prefix: Path)(implicit as: ActorSystem) =
    listPolicies(ListPoliciesRequest.withPrefix(prefix))

  override def listPolicies(request: ListPoliciesRequest)(implicit as: ActorSystem) =
    Source.single(request)
      .via(streaming.policyLister)
      .runWith(Sink.seq)

  override def listLocalPolicies()(implicit as: ActorSystem) =
  ...
  override def deletePolicyVersion(arn: PolicyArn, versionId: String)(implicit as: ActorSystem) =
    Source.single(DeletePolicyVersionRequest(arn, versionId))
      .via(streaming.policyVersionDeleter)
      .runWith(Sink.ignore)

  override def getPolicyVersion(arn: PolicyArn, versionId: String)(implicit as: ActorSystem) =
    Source.single(GetPolicyVersionRequest(arn, versionId))
      .via(streaming.policyVersionGetter)
      .runWith(Sink.head)

  override def listPolicyVersions(arn: PolicyArn)(implicit as: ActorSystem) =
    Source.single(arn)
      .via(streaming.policyVersionLister)
      .runWith(Sink.seq)

  override def setDefaultPolicyVersion(arn: PolicyArn, versionId: String)(implicit as: ActorSystem) =
