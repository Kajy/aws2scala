...
object KmsScalaCheckImplicits {
  ...
  implicit lazy val shrinkCreateKeyWithAliasRequest: Shrink[CreateKeyWithAliasRequest] =
    Shrink { request =>
      Shrink.shrink(request.alias)
        ...
        .map(x => request.copy(alias = x)) lazyAppendedAll
        Shrink.shrink(request.policy).map(x => request.copy(policy = x)) lazyAppendedAll
  ...
  implicit lazy val shrinkKeyMetadata: Shrink[KeyMetadata] =
    Shrink { metadata =>
      Shrink.shrink(metadata.arn).map(x => metadata.copy(arn = x, account = x.account, id = x.id)) lazyAppendedAll
        Shrink.shrink(metadata.creationDate).map(x => metadata.copy(creationDate = x)) lazyAppendedAll
        Shrink.shrink(metadata.enabled).map(x => metadata.copy(enabled = x)) lazyAppendedAll
        Shrink.shrink(metadata.description).map(x => metadata.copy(description = x)) lazyAppendedAll
        Shrink.shrink(metadata.usage).map(x => metadata.copy(usage = x)) lazyAppendedAll
        Shrink.shrink(metadata.state).map(x => metadata.copy(state = x)) lazyAppendedAll
