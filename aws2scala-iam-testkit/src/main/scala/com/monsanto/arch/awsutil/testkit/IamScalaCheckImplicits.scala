...
object IamScalaCheckImplicits {
  ...
  implicit lazy val shrinkAttachRolePolicyRequest: Shrink[AttachRolePolicyRequest] =
    Shrink { request =>
      Shrink.shrink(request.policyArn).map(x => request.copy(policyArn = x)) lazyAppendedAll
  ...
  implicit lazy val shrinkCreateRoleRequest: Shrink[CreateRoleRequest] =
    Shrink { request =>
      Shrink.shrink(request.name).filter(_.nonEmpty).map(x => request.copy(name = x)) lazyAppendedAll
        Shrink.shrink(request.assumeRolePolicy)
          .filterNot(p => p == request.assumeRolePolicy)
          .map(policy => request.copy(assumeRolePolicy = policy)) lazyAppendedAll
  ...
  implicit lazy val shrinkDetachRolePolicyRequest: Shrink[DetachRolePolicyRequest] =
    Shrink { request =>
      Shrink.shrink(request.roleName).filter(_.nonEmpty).map(x => request.copy(roleName = x)) lazyAppendedAll
  ...
  implicit lazy val shrinkRole: Shrink[Role] = {
    Shrink { role =>
      ...
      shrunkByName lazyAppendedAll shrunkByPath lazyAppendedAll shrunkByPolicy
    }
  }
  ...
  implicit lazy val shrinkUser: Shrink[User] =
    Shrink { user =>
      ...
      shrunkByName lazyAppendedAll shrunkByPath lazyAppendedAll shrunkByPasswordLastUsed
    }
  ...
  implicit lazy val shrinkCreatePolicyRequest: Shrink[CreatePolicyRequest] =
    Shrink { request =>
      Shrink.shrink(request.name).filter(_.nonEmpty).map(n => request.copy(name = n)) lazyAppendedAll
        Shrink.shrink(request.document).map(d => request.copy(document = d)) lazyAppendedAll
        Shrink.shrink(request.path).map(p => request.copy(path = p)) lazyAppendedAll
  ...
  implicit lazy val shrinkManagedPolicy: Shrink[ManagedPolicy] =
    Shrink { policy =>
      Shrink.shrink(policy.arn).map(a => policy.copy(name = a.name, arn = a, path = a.path)) lazyAppendedAll
        shrinkPolicyVersionId(policy.defaultVersionId).map(x => policy.copy(defaultVersionId = x)) lazyAppendedAll
        Shrink.shrink(policy.attachmentCount).filter(_ >= 0).map(x => policy.copy(attachmentCount = x)) lazyAppendedAll
  ...
  implicit lazy val shrinkManagedPolicyVersion: Shrink[ManagedPolicyVersion] =
    Shrink { policyVersion =>
      Shrink.shrink(policyVersion.document).map(x => policyVersion.copy(document = x)) lazyAppendedAll
  ...
  implicit lazy val shrinkCreatePolicyVersionRequest: Shrink[CreatePolicyVersionRequest] =
    Shrink { request =>
      Shrink.shrink(request.arn).map(x => request.copy(arn = x)) lazyAppendedAll
  ...
  private implicit lazy val shrinkPolicyArnAndVersion: Shrink[(PolicyArn, String)] =
    Shrink { arnAndVersion =>
      ...
      Shrink.shrink(arn).map(x => (x, version)) lazyAppendedAll
